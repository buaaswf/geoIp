...
void toGray(complex<float> *twodfourier,int pwid,int phei,char* pixval)
{
		...
		for(size_t p = 0;p<pwid*phei;p++){
			vals[p]=log(1+sqrt(pow(twodfourier[p].real(),2.0f)+pow(twodfourier[p].imag(),2.0f)));//对数级的幅度铺
			if(vals[p] > max){
				max = vals[p];
			}
			if(vals[p] < min){
...
void FourierFilter2::lowpass_trans(double thresold)
{
	...
	unsigned char * charbuf = new unsigned char[x*y];
	for (int i = 0; i < y; i++){
		for (int j = 0; j < x; j++)
		{
			complex<float> val=tmp[i*x + j];
			charbuf[i*x + j] = (unsigned char)(val.real()*255.0);
		}
	}
...
void FourierFilter3::lowpass_trans(double thresold,void (*ProgressChanged)(int ,int ,int ,bool &))
{
	...
	tmp = threeDIFFT(tmp, p_width, p_height, p_depth,ProgressChanged);
	unsigned char * charbuf = new unsigned char[x*y*z]; 
	for (int i = 0; i < z; i++){
		for (int j = 0; j < y;j++)
		{
			for (int k = 0; k < x;k++)
			{
				charbuf[i*y*x + j*x+ k] = (unsigned char)(tmp[i*p_height*p_width + j*p_width + k].real()*256.0);
			}
		}
	}
