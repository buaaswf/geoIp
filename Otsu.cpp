...
int Otsu_2(int arrays[100][100]){
	...
	for(k=Min_pix+1;k<Max_pix;k++){
		...
		w1=pix_number/10000;//前景像素点数占整幅图像的比例
...
int Otsu_3(int arrays[60][60][60]){
	...
	for(k=Min_pix+1;k<Max_pix;k++){
		...
		w1=pix_number1/(pix_number1+pix_number2);//前景像素点数占整幅图像的比例
		w2=1-w1;//背景素点数占整幅图像的比例
        g=w1*w2*(u1-u2)*(u1-u2);
		if(M_g<g){
			M_g=g;
...
int Cfc(char *image){
	...
	for(k=Min_pix;k<Max_pix;k++){
		...
		s1=0.0;
		for(i=Min_pix;i<=k;i++){
          s1+=Pix_array[i]*(i-u1)*(i-u1)/pix_number;//计算前景方差
		}
		w2=1-w1;//背景素点数占整幅图像的比例
		pix_number=0.0;
		Pix_val=0.0;
		...
		for(i=k+1;i<=Max_pix;i++){
          s2+=Pix_array[i]*(i-u2)*(i-u2)/pix_number;//计算背景方差
		}
		s=1/(s1+s2);
		if(M_s<s){
			M_s=s;
...
int Otsu_test(char *image){
	...
	//t=t*(1-fabs(2*w1-1));
	cout<<"w="<<w1<<endl;
	t=(int)(t*(1-fabs(1-2*w1)));////考虑面积比值的影响
  return t;
}
...
int Otsu_2D(char *image){//二维OTSU算法
	double w1;//前景的像素点数占整幅图像的比例
	...
	for(i=0;i<SIZE;i++){
		...
		for(j=-1;j<2;j++){//在Z轴上平滑
			z=(i+j)/(X_VALUE*Y_VALUE);//Z轴
			z=z<0?0:z;
			z=z>(Z_VALUE-1)?(Z_VALUE-1):z;
			for(k=-1;k<2;k++){//在Y轴上平滑
				y=(i-z*X_VALUE*Y_VALUE+k)/X_VALUE;//Y轴
				y=y<0?0:y;
				y=y>(Y_VALUE-1)?(Y_VALUE-1):y;
				for(l=-1;l<2;l++){//在X轴上平滑
				    x=i-z*X_VALUE*Y_VALUE-y*X_VALUE+l;//X轴
					x=x<0?0:x;
					x=x>(X_VALUE-1)?(X_VALUE-1):x;
					Pix_val_k+=*(image+z*X_VALUE*Y_VALUE+y*X_VALUE+x);//计算立方体27个像素的值
				}
			}
		}
...
int Otsu_2D_Step(char *image){//改进的二维OTSU算法
	double w1;//前景的像素点数占整幅图像的比例
	double w1_k;//前景的k邻域像素点数占整幅图像的比例
	double u1;//前景的平均灰度
	double u1_k;//前景的k邻域平均灰度
